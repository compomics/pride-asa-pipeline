...
public class PrideAsapSearchParamExtractorTest extends TestCase {
     ...
     * System.out.println("testGetSearchParameters"); File temp =
     * File.createTempFile("temp", ".tmp"); temp.deleteOnExit(); File
     * outputFolder = File.createTempFile("temp", ".tmp").getParentFile(); File
     * outputMGFFolder = new File(outputFolder, "mgf"); PrideAsapExtractor
     * instance = new PrideAsapExtractor("3", outputFolder); SearchParameters
     * identificationParameters = instance.inferSearchParameters();
     ...
     *
     * assertEquals(identificationParameters.getPrecursorAccuracyType(),
     ...
     *
     * assertTrue(identificationParameters.getPtmSettings().getVariableModifications().contains("Phosphorylation
     * of S"));
     * assertTrue(identificationParameters.getPtmSettings().getVariableModifications().contains("Oxidation
     * of M"));
     * assertTrue(identificationParameters.getPtmSettings().getFixedModifications().contains("Carbamidomethylation
     * of C")); }
     */
}
