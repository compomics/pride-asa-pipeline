...
public class JdbcExperimentRepository extends JdbcDaoSupport implements ExperimentRepository {
    ...
    private static final String SELECT_EXPERIMENT_ACCESSIONS = new StringBuilder().append("select exp.accession as accession, exp.title as title ").append("from pride_experiment exp").append(" order by exp.accession+0 asc").toString();
    private static final String SELECT_EXPERIMENT_ACCESSIONS_BY_TAXONOMY = new StringBuilder().append("select exp.accession as accession, exp.title as title ").append("from pride_experiment exp, mzdata_sample_param sample ").append("where sample.parent_element_fk = exp.mz_data_id ").append("and sample.accession = ? ").append(" order by exp.accession+0 asc").toString();
    //PSI:1000040 is "Mass to Charge Ratio" and
    //PSI:1000041 is "Charge State"
    private static final String SELECT_EXPERIMENT_IDENTIFICATIONDATA = new StringBuilder().append("select main.*, par1.value as mz, par2.value as charge_state from ( ").append("select spec.spectrum_identifier, ").append("spec.spectrum_id, ").append("exp.accession, ").append("mzdata.mz_data_id, ").append("pep.sequence, ").append("pep.peptide_id, ").append("prec.precursor_id ").append("from ").append("pride_experiment exp, ").append("mzdata_mz_data mzdata, ").append("pride_identification iden, ").append("pride_peptide pep, ").append("mzdata_spectrum spec, ").append("mzdata_precursor prec ").append("where ").append("exp.accession = ? and ").append("exp.mz_data_id = mzdata.mz_data_id and ").append("exp.experiment_id = iden.experiment_id and ").append("iden.identification_id = pep.identification_id and ").append("pep.spectrum_ref = spec.spectrum_identifier and ").append("mzdata.mz_data_id = spec.mz_data_id and ").append("spec.spectrum_id = prec.spectrum_id ").append(") main ").append("left join mzdata_ion_selection_param par1 on par1.parent_element_fk = main.precursor_id and par1.accession = 'PSI:1000040' ").append("left join mzdata_ion_selection_param par2 on par2.parent_element_fk = main.precursor_id and par2.accession = 'PSI:1000041' ").toString();
    ...
    private static final String SELECT_SPECTRA_METADATA = new StringBuilder().append("select ").append("main.spectrum_id, par1.value as precursor_mz, par2.value as precursor_charge_state ").append("from ").append("( ").append("   select ").append("   spec.spectrum_id, prec.precursor_id ").append("   from pride_experiment exp, ").append("   mzdata_mz_data mzdata, ").append("   mzdata_spectrum spec, ").append("   mzdata_precursor prec ").append("   where exp.mz_data_id = mzdata.mz_data_id ").append("   and mzdata.mz_data_id = spec.mz_data_id ").append("   and spec.spectrum_id = prec.spectrum_id ").append("   and exp.accession = ? ").append(") ").append("as main ").append("left join mzdata_ion_selection_param par1 on par1.parent_element_fk = main.precursor_id ").append("and par1.accession = 'PSI:1000040' ").append("left join mzdata_ion_selection_param par2 on par2.parent_element_fk = main.precursor_id ").append("and par2.accession = 'PSI:1000041' ").append("; ").toString();
    ...
    public List<Identification> loadExperimentIdentifications(String experimentAccession) {
        ...
        List<Identification> identifications = getJdbcTemplate().query(SELECT_EXPERIMENT_IDENTIFICATIONDATA, new IdentificationDataMapper(), new Object[]{experimentAccession});
